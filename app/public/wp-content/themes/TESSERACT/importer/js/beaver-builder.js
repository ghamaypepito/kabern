...
//alert(123);
 $(window).load(function(){
	 $(document).on('click', ".tabs-menu a", function(event) {
        ...
        $(".tab-contentx").not(tab).css("display", "none");
        $(tab).fadeIn();
		if(!$(this).hasClass('txt-replace')){
			$(".tabs-menu a.txt-replace").html('More');
			//$('.sub-menux').slideUp();
		}
    });
	
	$(document).on('click', ".dropdown", function(){
		$('.sub-menux').slideToggle();
	});
	
	$(document).on('click', ".sub-menux li", function() {
    $(".txt-replace").html($(this).find('a').html());
    });
	
	
	$(document).on('click', ".fl-lightbox-mask", function() {
    $('.fl-builder-tesseract-blocks-lightbox').hide();
    });
	...
	$( '#elementor-template-library-menu-my-templates' ).after(
	...
	$( document ).on( 'click', '.fl-builder-tesseract-blocks-button', function() {
		...
		$.post( ajaxurl, {action: 'tesseract_add_button_to_page_builder'}, function( blocksHtml ) {
				...
				contentBlocksLightbox.open( $( blocksHtml ).html() );
