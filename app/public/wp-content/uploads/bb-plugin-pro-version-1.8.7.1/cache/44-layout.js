...
{$('body').off('uabb-trigger.'+hook,callback);},};})(jQuery);jQuery(document).ready(function($){var uabb_browser=bowser.name,uabb_browser_v=bowser.version,uabb_browser_class=uabb_browser.replace(/\s+/g,'-').toLowerCase(),uabb_browser_v_class=uabb_browser_class+parseInt(uabb_browser_v);$('html').addClass(uabb_browser_class).addClass(uabb_browser_v_class);});var wpAjaxUrl='https://kabernenterprises.com/wp-admin/admin-ajax.php';var flBuilderUrl='https://kabernenterprises.com/wp-content/plugins/bb-plugin-pro-version-1.8.7.1/';var FLBuilderLayoutConfig={anchorLinkAnimations:{duration:1000,easing:'swing',offset:100},paths:{pluginUrl:'https://kabernenterprises.com/wp-content/plugins/bb-plugin-pro-version-1.8.7.1/',wpAjaxUrl:'https://kabernenterprises.com/wp-admin/admin-ajax.php'}};(function($){if(typeof FLBuilderLayout!='undefined'){return;}
...
{var field=$(this);field.removeClass('fl-form-error');field.siblings('.fl-form-error-message').hide();}};$(function(){FLBuilderLayout.init();});})(jQuery);(function($){$(function(){$('.fl-map').on('click',function(){$(this).find('iframe').css('pointer-events','auto');});});})(jQuery);function initMap(map,position,zoom){map.setCenter(position);map.setZoom(zoom);var marker=new google.maps.Marker({position:position,map:map});}
var position={lat:-1.2920659,lng:36.82194619999996};initMap(new google.maps.Map(document.getElementById('map-module-577eb91e4267e'),{scrollwheel:false}),position,9);(function($){UABBContactForm=function(settings)
{this.settings=settings;this.nodeClass='.fl-node-'+settings.id;this.ajaxurl=settings.uabb_ajaxurl;this.name_required=settings.name_required;this.email_required=settings.email_required;this.subject_required=settings.subject_required;this.phone_required=settings.phone_required;this.msg_required=settings.msg_required;this._init();};UABBContactForm.prototype={settings:{},nodeClass:'',ajaxurl:'',name_required:'no',email_required:'yes',subject_required:'no',phone_required:'no',msg_required:'no',_init:function()
...
{var theForm=$(this.nodeClass+' .uabb-contact-form'),submit=$(this.nodeClass+' .uabb-contact-form-submit'),name=$(this.nodeClass+' .uabb-name input'),email=$(this.nodeClass+' .uabb-email input'),phone=$(this.nodeClass+' .uabb-phone input'),subject=$(this.nodeClass+' .uabb-subject input'),message=$(this.nodeClass+' .uabb-message textarea'),mailto=$(this.nodeClass+' .uabb-mailto'),ajaxurl=this.ajaxurl,email_regex=/\S+@\S+\.\S+/,phone_regex=/^[ 0-9.()\[\]+-]*$/,isValid=true;postId=theForm.closest('.fl-builder-content').data('post-id'),templateId=theForm.data('template-id'),templateNodeId=theForm.data('template-node-id'),nodeId=theForm.closest('.fl-module').data('node');e.preventDefault();name.on('focus',this._removeErrorClass);email.on('focus',this._removeErrorClass);phone.on('focus',this._removeErrorClass);phone.on('keyup',this._removeExtraSpaces);subject.on('focus',this._removeErrorClass);message.on('focus',this._removeErrorClass);if(submit.hasClass('uabb-disabled')){return;}
